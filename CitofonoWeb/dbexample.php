...
<p>
<?php
$insert="insert into `acl` (`badge_code`, `allowed`, `sched`) values ";

touch(config::localdb);
try{
	$local = new PDO('sqlite:'.config::localdb);
	$query="CREATE TABLE IF NOT EXISTS acl (badge_code varchar(50) PRIMARY KEY, allowed tinyint, sched text)";
	$local->exec($query);
}
